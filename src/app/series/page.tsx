...
import { PlayIcon, StarIcon, TvIcon, ChevronDownIcon, ChevronLeftIcon, XMarkIcon, BellIcon, ArrowDownTrayIcon, UserIcon, UserCircleIcon, Cog6ToothIcon, ArrowRightOnRectangleIcon, ChatBubbleLeftRightIcon, HeartIcon, MagnifyingGlassIcon } from '@heroicons/react/24/outline'
...
export default function SeriesPage() {
  ...
  useEffect(() => {
    ...
    AnalyticsService.trackCatalogueView('Séries Populaires', user?.id)
  ...
  const applyFilters = () => {
    ...
    if (filters.genre !== 'all') {
      filtered = filtered.filter(serie => 
        GenreService.contentMatchesGenre(serie, filters.genre)
      )
    }
    ...
    if (filters.year !== 'all') {
      ...
      filtered = filtered.filter(serie => serie.year === year)
    }
  ...
  const getUniqueGenres = () => {
    return GenreService.getAllUniqueGenres([], series)
  }

  const getUniqueYears = () => {
    const years = Array.from(new Set(series.map(serie => serie.year)))
      .sort((a, b) => b - a)
    return years
  }
  ...
  const loadContent = async () => {
    try {
      setIsLoading(true)
      const [seriesData, homepageData] = await Promise.all([
  ...
  const handleContentMouseEnter = (content: Series, e: React.MouseEvent<HTMLElement>) => {
  ...
  const handleContentMouseLeave = (content: Series, e: React.MouseEvent<HTMLElement>) => {
    // Nettoyer la référence de l'élément
    cardElementsRef.current.delete(content.id)
  }

  const handleContentClick = (content: Series, e: React.MouseEvent<HTMLElement>) => {
  ...
  const handleContentTouch = (content: Series, e: React.TouchEvent<HTMLElement>) => {
    if (user && content.trailerUrl) {
      e.preventDefault()
      e.stopPropagation()
  ...
  const shouldShowExpandableBefore = (index: number) => {
    ...
    const items = getCurrentPageItems(filteredSeries)
      ...
      <ProtectedRoute>
        <div className="min-h-screen bg-gradient-to-br from-dark-300 via-dark-200 to-dark-100 flex items-center justify-center">
          <div className="text-center">
            ...
            ></div>
            <p className="text-gray-400">Chargement des séries...</p>
          </div>
        </div>
      </ProtectedRoute>
    )
        ...
        <main className="px-8 py-4 pt-14 sm:pt-18 md:pt-20 lg:pt-14">
          <div>
            ...
            <div className="flex items-center space-x-3 sm:space-x-4 mb-4 sm:mb-5 md:mb-6">
              ...
              <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-white">Toutes les Séries</h1>
            </div>

            {/* Grille des séries */}
            {getCurrentPageItems(filteredSeries).length > 0 ? (
              <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-9 gap-3 sm:gap-4 auto-rows-min">
                {getCurrentPageItems(filteredSeries).map((serie, index) => {
                  const clickedSerie = expandedCardId ? getCurrentPageItems(filteredSeries).find(m => m.id === expandedCardId) : null
                  return (
                    <React.Fragment key={serie.id}>
                      {/* Afficher la section extensible avant la première carte de la rangée si c'est la dernière rangée */}
                      {shouldShowExpandableBefore(index) && clickedSerie && (
                        <ExpandableCardSection
                          content={clickedSerie}
                          isOpen={expandedCardId === clickedSerie.id}
                  ...
                  <Link
                    ...
                    onMouseEnter={(e) => handleContentMouseEnter(serie, e)}
                    onMouseLeave={(e) => handleContentMouseLeave(serie, e)}
                  ...
                  >
                    ...
                    <div className="mt-2 sm:mt-2.5">
                      <h3 className="text-white font-medium text-base sm:text-lg md:text-xl truncate mb-1">{serie.title}</h3>
                      <div className="flex items-center space-x-1.5 sm:space-x-2 text-sm sm:text-base text-gray-400">
                        ...
                        <div className="flex items-center space-x-1">
                          ...
                          <span>
                            {(() => {
                              const stats = ReviewsService.getReviewStats(serie.id)
                  ...
                  {shouldShowExpandableAfter(index) && clickedSerie && (
                    <ExpandableCardSection
                      content={clickedSerie}
                      isOpen={expandedCardId === clickedSerie.id}
                      onClose={() => setExpandedCardId(null)}
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-bold text-white mb-2">Aucune série trouvée</h3>
