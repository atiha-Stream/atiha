...
import { PlayIcon, StarIcon, TrophyIcon, ChevronDownIcon, ChevronLeftIcon, ChatBubbleLeftRightIcon, HeartIcon, XMarkIcon, BellIcon, ArrowDownTrayIcon, UserIcon, UserCircleIcon, Cog6ToothIcon, ArrowRightOnRectangleIcon } from '@heroicons/react/24/outline'
...
export default function SportsPage() {
  ...
  useEffect(() => {
    ...
    AnalyticsService.trackCatalogueView('Sports', user?.id)
  ...
  const loadContent = async () => {
    try {
      ...
      // Filtrer pour ne garder que le contenu de catalogue "sports"
      const sportsMovies = moviesData.filter(movie => 
        movie.catalogue === 'sports'
      )
      
      const sportsSeries = seriesData.filter(serie => 
        serie.catalogue === 'sports'
      )
      
      setMovies(sportsMovies)
      setSeries(sportsSeries)
      setHomepageContent(HomepageContentService.getContent())
      // Initialiser filteredContent avec tous les contenus
      const allContent = [...sportsMovies, ...sportsSeries]
      ...
      <ProtectedRoute>
        <div className="min-h-screen bg-gradient-to-br from-dark-300 via-dark-200 to-dark-100 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-white text-lg">Chargement des sports...</p>
          </div>
        </div>
      </ProtectedRoute>
    )
        ...
        <main className="px-8 py-4 pt-14 sm:pt-18 md:pt-20 lg:pt-14">
          <div>
            ...
            <div className="flex items-center space-x-3 sm:space-x-4 mb-4 sm:mb-5 md:mb-6">
              ...
              <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-white">Sports</h1>
            </div>
            ...
            {getCurrentPageItems(filteredContent).length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-bold text-white mb-2">Aucun contenu sportif trouvé</h3>
                <p className="text-gray-400">
                  Aucun contenu sportif ne correspond à vos critères de recherche.
                </p>
              </div>
            )}
